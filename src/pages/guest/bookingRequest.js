...
export default function BookingRequest() {
  ...
  const handlePrice = ({outputType, selectedDate}) => {
    if (outputType === 'totalNightPrice') {
      if (selectedDate) {
        let dateDifference = new Date(selectedDate.endDate) - new Date(selectedDate.startDate);
        let totalNights = dateDifference / 1000 / 60 / 60 / 24;
        return (selectedListing.roomrate * totalNights);
      };
    } else if (outputType === 'bookingFee') {
      let bookingFee = handlePrice({outputType: 'totalNightPrice', selectedDate}) * 0.025;
      return (bookingFee)
    } else if (outputType === 'vat') {
      let vat = handlePrice({outputType: 'totalNightPrice', selectedDate}) * 0.1;
      return (vat)
    } else if (outputType === 'bookingTotal') {
      let bookingTotal = handlePrice({outputType: 'totalNightPrice', selectedDate}) + handlePrice({outputType: 'bookingFee', selectedDate}) + handlePrice({outputType: 'vat', selectedDate});
      return (bookingTotal);
    };
  }
  ...
  useEffect(() => {
    const fetchListing = async () => {
      if (propertyId) {
        try {
          let listing = await axios.get(`${BACKEND_URL}/properties/${propertyId}`);
                    ...
                    <div className="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                      <div className="relative sm:grid sm:grid-cols-1 sm:gap-x-6 sm:pr-0">
                        <div className="flex flex-col gap-2">
                          ...
                          <p className=" text-gray-500 sm:mt-2 sm:block text-xs">
                            <span className="font-bold text-gray-800">Configuration:</span>{" "}
                            {selectedListing.configuration}
                          </p>
                          <p className=" text-gray-500 sm:mt-2 sm:block text-xs">
                            <span className="font-bold text-gray-800">Amenities:</span>{" "}
                            {selectedListing.amenities}
                          </p>
                          <br />
                          <dl className="grid flex-1 grid-cols-2 gap-x-6 text-sm sm:col-span-2 sm:grid-cols-2 lg:col-span-2">
                            <div>
                              <dt className="font-medium text-gray-900">Selected Dates:</dt>
                              <dd className="mt-1 text-gray-500">
                                <time dateTime={selectedDate.startDate}>{new Date(selectedDate.startDate).toDateString()} - <span className="sm:hidden"><br /></span> {new Date(selectedDate.endDate).toDateString()}</time>
                              </dd>
                            </div>
                            <div>
                              <dt className="font-medium text-gray-900">Guests: </dt>
                              <dd className="mt-1 text-gray-500">{adultNumber} Adult, {childrenNumber} Children</dd>
                            </div>
                          </dl>
                        </div>
                      </div>
            ...
            >
              ...
              <dl className="mt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <dt className="text-sm text-gray-600">¥ {selectedListing.roomrate} x {totalNights} nights</dt>
                  <dd className="text-sm font-medium text-gray-900">¥ {handlePrice({outputType: 'totalNightPrice', selectedDate})}</dd>
                </div>
                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                  ...
                  <dd className="text-sm font-medium text-gray-900">¥ {handlePrice({outputType: 'bookingFee', selectedDate})}</dd>
                </div>
                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                  ...
                  <dd className="text-sm font-medium text-gray-900">¥ {handlePrice({outputType: 'vat', selectedDate})}</dd>
                </div>
                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                  <dt className="text-base font-medium text-gray-900">Booking total</dt>
                  <dd className="text-base font-medium text-gray-900">¥ {handlePrice({outputType: 'bookingTotal', selectedDate})}</dd>
                </div>
              </dl>
    ...
    <>
      <header>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold leading-tight tracking-tight text-gray-900">Booking Request</h1>
        </div>
      </header>
